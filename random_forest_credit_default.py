...
OUTPUT_DIR = BASE_DIR / "random_forest_outputs"
...
def plot_pr_curve(y_true: pd.Series, y_prob: np.ndarray, save_path: Path) -> None:
    ...
    plt.figure(figsize=(7, 5))
    plt.plot(recall, precision, label=f"Random Forest (PR-AUC = {pr_auc:.4f})")
    plt.xlabel("Recall")
...
def main() -> None:
    ...
    print("Retraining best Random Forest pipeline on full training data...")
    ...
    pred_df = pd.DataFrame(
        {
            ID_COL: test_ids,
            "random_forest_default_probability": test_prob,
            "random_forest_predicted_label": test_pred,
        }
    )
    pred_df.to_csv(OUTPUT_DIR / "random_forest_test_predictions.csv", index=False)
