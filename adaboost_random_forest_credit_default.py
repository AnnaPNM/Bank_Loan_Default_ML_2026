...
OUTPUT_DIR = BASE_DIR / "adaboost_random_forest_outputs"
...
def plot_roc_curve(y_true: pd.Series, y_prob: np.ndarray, save_path: Path) -> None:
    ...
    plt.plot(fpr, tpr, label=f"AdaBoost + RF (ROC-AUC = {roc_auc:.4f})")
...
def plot_pr_curve(y_true: pd.Series, y_prob: np.ndarray, save_path: Path) -> None:
    ...
    plt.figure(figsize=(7, 5))
    plt.plot(recall, precision, label=f"AdaBoost + RF (PR-AUC = {pr_auc:.4f})")
    plt.xlabel("Recall")
...
def plot_feature_importance(
    ...
    plt.title(f"Top {top_n} AdaBoost + RF Feature Importances")
    plt.tight_layout()
    plt.savefig(save_path, dpi=200)
    plt.close()
...
def main() -> None:
    ...
    param_grid = {
        ...
        "model__estimator__n_estimators": [30, 60],
        "model__estimator__max_depth": [None, 10],
        "model__estimator__min_samples_leaf": [1, 20],
        "model__estimator__max_features": ["sqrt"],
    }

    cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=RANDOM_STATE)

    print("Running GridSearchCV for AdaBoost with Random Forest base learner...")
    grid = GridSearchCV(
        estimator=pipeline,
    ...
    print("Retraining best AdaBoost + RF pipeline on full training data...")
    ...
    pred_df = pd.DataFrame(
        {
            ID_COL: test_ids,
            "adaboost_rf_default_probability": test_prob,
            "adaboost_rf_predicted_label": test_pred,
        }
    )
    pred_df.to_csv(OUTPUT_DIR / "adaboost_rf_test_predictions.csv", index=False)
